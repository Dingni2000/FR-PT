...
class SimpleCNN_nette_Base(nn.Module, ABC):
    def __init__(self, activate=torch.tanh, version="base"):
        super().__init__()
        self.activate = activate
        self._version = version
        self.name = f"nette_simplecnn{version}_{self.activate.__name__}"
        self.recons_map = {}
...
class SimpleCNN_nette_v1(SimpleCNN_nette_Base):
    def __init__(self, activate=torch.tanh):
        ...
        self.recons_map = { 'recons_out': 7, 'recons_z6': 6, 'recons_z5': 5, 'recons_z4': 4,
            'recons_z3': 3, 'recons_z2': 2, 'recons_z1': 1,}
    ...
    def get_recons_fea(self, input_data, label_data, conv_method='fft_pad', recons_key=None, device='cuda:0'):
        ...
        return {'recons_out': recons_out, 'recons_z6': recons_z6, 'recons_z5': recons_z5,
            'recons_z4': recons_z4, 'recons_z3': recons_z3, 'recons_z2': recons_z2, 'recons_z1': recons_z1}


class SimpleCNN_nette_v2(SimpleCNN_nette_Base):
    def __init__(self, activate=torch.tanh):
    ...
    def get_recons_fea(self, input_data, label_data, conv_method='fft_pad', recons_key=None, device='cuda:0'):
        ...
        recons_a4_p = convo_reverseCom(self.pooling(a4), recons_z5, self.conv5, method=conv_method)
        recons_a4 = pooling_reverseCom(recons_a4_p, a4, self.pooling)
        recons_z4 = route_rvs_act(self.activate, res['z4'], recons_a4)
        if recons_key == 'recons_z4': return recons_z4

        a3 = self.activate(res['z3'])
        recons_a3_p = convo_reverseCom(self.pooling(a3), recons_z4, self.conv4, method=conv_method)
        recons_a3 = pooling_reverseCom(recons_a3_p, a3, self.pooling)
        recons_z3 = route_rvs_act(self.activate, res['z3'], recons_a3)
        if recons_key == 'recons_z3': return recons_z3

        a2 = self.activate(res['z2'])
        recons_a2_p = convo_reverseCom( self.pooling(a2), recons_z3, self.conv3, method=conv_method)
        recons_a2 = pooling_reverseCom(recons_a2_p, a2, self.pooling)
        recons_z2 = route_rvs_act(self.activate, res['z2'], recons_a2)
        if recons_key == 'recons_z2': return recons_z2

        a1 = self.activate(res['z1'])
        recons_a1_p = convo_reverseCom(self.pooling(a1), recons_z2, self.conv2, method=conv_method)
        recons_a1 = pooling_reverseCom(recons_a1_p, a1, self.pooling)
        recons_z1 = route_rvs_act(self.activate, res['z1'], recons_a1)
        if recons_key == 'recons_z1': return recons_z1
        return {'recons_out': recons_out, 'recons_z6': recons_z6, 'recons_z5': recons_z5,
                'recons_z4': recons_z4, 'recons_z3': recons_z3, 'recons_z2': recons_z2,
                'recons_z1': recons_z1}


def SimpleCNN_nette(activate=torch.tanh, version="v1"):
    version = str(version).lower()
    if version in ("1", "v1", "simplecnnv1"):
        return SimpleCNN_nette_v1(activate=activate)
    if version in ("2", "v2", "simplecnnv2"):
        return SimpleCNN_nette_v2(activate=activate)
...
class ResNet18_nette(ReconstructableResNet):
    def __init__(self, pretrain=False):
        super().__init__(task_name="nette", num_classes=10, arch="18", input_channels=3, pretrain=pretrain)


class ResNet34_nette(ReconstructableResNet):
    def __init__(self, pretrain=False):
        super().__init__(task_name="nette", num_classes=10, arch="34", input_channels=3, pretrain=pretrain)


def ResNet_nette(version="18", pretrain=False):
    version = str(version).lower().replace("resnet", "")
    if version in (18, "18", "v18"):
        return ResNet18_nette(pretrain=pretrain)
    if version in (34, "34", "v34"):
        return ResNet34_nette(pretrain=pretrain)
    raise ValueError("version must be one of: '18', '34', 'resnet18', 'resnet34'")


if __name__ == '__main__':
    for version in (1, 2):
        model = SimpleCNN_nette(torch.tanh, version=version)
