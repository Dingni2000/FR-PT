...
def _iterative_feature_reliable(z, anchor, max_abs=1e3, max_deviation_ratio=1e2):
    ...
    if max_deviation_ratio is not None:
        ...
        floor = 1e-4 * math.sqrt(af.shape[1])
...
def _linearized_reverse_map(
    ...
    info = { "history": history, "methods": methods, "reliability": reliability,
...
def _vjp_reverse_map(
    ...
    for _ in range(steps):
        ...
        with torch.no_grad():
            ...
            delta = z - prev
            norms = torch.linalg.vector_norm(delta.reshape(delta.shape[0], -1), dim=1)
            step_norms.append(float(norms.max().detach().cpu()))

            if max_step_norm is not None:
                scale = (max_step_norm / norms.clamp_min(1e-12)).clamp(max=1.0)
                z.copy_(prev + delta * scale.view(-1, *([1] * (delta.ndim - 1))))
...
if __name__ == "__main__":
    ...
    from imagecls.models.simplevit_recons import ReconstructableSimpleViTv2

    torch.manual_seed(0)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    def check_reverse(name, block, front, target, **kwargs):
        was_training = block.training
        block.eval()
        with torch.no_grad():
            initial = _relative_flat_norm(block(front) - target, target).amax()
        reconstructed, info = composite_reverseCom(
            front, target, block, return_info=True, **kwargs)
        with torch.no_grad():
            final = _relative_flat_norm(block(reconstructed) - target, target).amax()
        block.train(was_training)
        assert torch.isfinite(reconstructed).all(), f"{name}: non-finite reconstruction"
        assert final < initial, f"{name}: reconstruction did not improve ({initial:.3e} -> {final:.3e})"
        assert info["selected_method"] == "linearized"
        print(f"[PASS] {name}: {initial:.3e} -> {final:.3e}")

    resnet_block = resnet18(weights=None).to(device).layer1[0]
    resnet_block.train()
    resnet_true = torch.rand(1, 64, 4, 4, device=device) + 0.5
    resnet_front = resnet_true + 0.01 * torch.randn_like(resnet_true)
    resnet_block.eval()
    with torch.no_grad():
        resnet_target = resnet_block(resnet_true)
    resnet_block.train()
    check_reverse(
        "ResNet18 BasicBlock",
        resnet_block,
    ...
    assert resnet_block.training and all(param.requires_grad for param in resnet_block.parameters())

    vit = ReconstructableSimpleViTv2(
        task_name="test",
        image_size=4,
        patch_size=2,
        num_classes=2,
    ...
    vit_block = vit.block1
    vit_block.train()
    vit_true = 0.1 * torch.randn(1, 2, 4, device=device)
    vit_front = vit_true + 0.01 * torch.randn_like(vit_true)
    with torch.no_grad():
        vit_target = vit_block(vit_true)
    check_reverse(
        "SimpleViTv2 Pre-LN block",
        vit_block,
        vit_front,
        vit_target,
        iter=8,
    ...
    assert vit_block.training and all(param.requires_grad for param in vit_block.parameters())
