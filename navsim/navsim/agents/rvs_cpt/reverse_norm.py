...
if __name__ == "__main__":
    ...
    # ---------------- BatchNorm ----------------
    bn_cases = (
        ("BatchNorm1d", nn.BatchNorm1d(3), (2, 3, 5)),
        ("BatchNorm2d", nn.BatchNorm2d(3), (2, 3, 4, 5)),
        ("BatchNorm3d", nn.BatchNorm3d(3), (2, 3, 2, 4, 5)),
        ("BN affine=False", nn.BatchNorm2d(3, affine=False), (2, 3, 4, 5)),
    )
    for name, layer, shape in bn_cases:
        with torch.no_grad():
            layer.running_mean.copy_(torch.tensor([-1.0, 0.5, 2.0]))
            layer.running_var.copy_(torch.tensor([0.5, 2.0, 4.0]))
            if layer.affine:
                layer.weight.copy_(torch.tensor([1.0, 0.5, -2.0]))
                layer.bias.copy_(torch.tensor([0.2, -0.3, 0.4]))
        layer.eval()

        expected = torch.randn(shape)
        y = layer(expected)
        reconstructed = bn_reverseCom(torch.randn_like(expected), y, layer)
        torch.testing.assert_close(reconstructed, expected, atol=atol, rtol=rtol)
        print(f"[PASS] {name}")

    # Exact zero weight: the whole non-invertible channel is reconstructed as zero.
    bn = nn.BatchNorm2d(3).eval()
    with torch.no_grad():
        bn.running_mean.copy_(torch.tensor([-1.0, 0.5, 2.0]))
        bn.running_var.copy_(torch.tensor([0.5, 2.0, 4.0]))
        bn.weight.copy_(torch.tensor([1.0, 0.0, -2.0]))
        bn.bias.copy_(torch.tensor([0.2, -0.3, 0.4]))

    expected = torch.randn(2, 3, 4, 4)
    y = bn(expected)
    reconstructed = bn_reverseCom(torch.randn_like(expected), y, bn)
    torch.testing.assert_close(reconstructed[:, 1], torch.zeros_like(reconstructed[:, 1]))
    torch.testing.assert_close(bn(reconstructed), y, atol=atol, rtol=rtol)
    print("[PASS] BN zero-weight channel -> zero input")

    # Near-zero weight is also treated as non-invertible and never divided by.
    bn_small = nn.BatchNorm1d(3).eval()
    with torch.no_grad():
        bn_small.weight.copy_(torch.tensor([1.0, 1e-14, -1.0]))
    expected = torch.randn(2, 3, 5)
    y = bn_small(expected)
    reconstructed = bn_reverseCom(
        torch.randn_like(expected), y, bn_small, min_abs_weight=1e-12
    )
    assert torch.isfinite(reconstructed).all()
    torch.testing.assert_close(
        reconstructed[:, 1], torch.zeros_like(reconstructed[:, 1])
    )
    print("[PASS] BN near-zero weight -> zero input, finite reconstruction")

    # Impossible target on a zero-weight channel: output must fall back to bias.
    y_bad = y.clone()
    y_bad[:, 1] += 10.0
    reconstructed = bn_reverseCom(
        torch.randn_like(expected), y_bad, bn_small, min_abs_weight=1e-12
    )
    out = bn_small(reconstructed)
    torch.testing.assert_close(
        out[:, 1], bn_small.bias[1].expand_as(out[:, 1]), atol=atol, rtol=rtol
    )
    print("[PASS] BN impossible zero-weight target -> affine bias")

    # ---------------- LayerNorm ----------------
    ln = nn.LayerNorm(4)
    with torch.no_grad():
        ln.weight.copy_(torch.tensor([1.0, 0.5, -2.0, 0.75]))
        ln.bias.copy_(torch.tensor([0.2, -0.3, 0.4, 0.1]))

    expected = torch.randn(2, 3, 4)
    y = ln(expected)
    reconstructed = layer_norm_reverseCom(y, ln, front_fea=expected)
    torch.testing.assert_close(ln(reconstructed), y, atol=atol, rtol=rtol)
    print("[PASS] LayerNorm nonzero affine")

    ln_no_affine = nn.LayerNorm(4, elementwise_affine=False)
    expected = torch.randn(2, 3, 4)
    y = ln_no_affine(expected)
    reconstructed = layer_norm_reverseCom(y, ln_no_affine, front_fea=expected)
    torch.testing.assert_close(ln_no_affine(reconstructed), y, atol=atol, rtol=rtol)
    print("[PASS] LayerNorm affine=False")

    # One zero affine weight: final reconstructed input at that position is zero,
    # while all attainable LayerNorm outputs remain unchanged.
    ln_zero = nn.LayerNorm(4)
    with torch.no_grad():
        ln_zero.weight.copy_(torch.tensor([1.0, 0.0, -2.0, 0.5]))
        ln_zero.bias.copy_(torch.tensor([0.2, -0.3, 0.4, 0.1]))

    expected = torch.randn(2, 3, 4)
    y = ln_zero(expected)
    reconstructed = layer_norm_reverseCom(y, ln_zero, front_fea=expected)
    torch.testing.assert_close(
        reconstructed[..., 1], torch.zeros_like(reconstructed[..., 1]), atol=atol, rtol=0
    )
    torch.testing.assert_close(ln_zero(reconstructed), y, atol=atol, rtol=rtol)
    print("[PASS] LayerNorm zero weight -> zero reconstructed position")

    # Multiple zero weights: minimum-norm completion makes all missing normalized
    # entries equal, so the free LayerNorm shift can set every such input to zero.
    ln_multi = nn.LayerNorm(4)
    with torch.no_grad():
        ln_multi.weight.copy_(torch.tensor([0.0, 1.0, 0.0, -2.0]))
        ln_multi.bias.copy_(torch.tensor([0.2, -0.3, 0.4, 0.1]))

    expected = torch.randn(2, 3, 4)
    y = ln_multi(expected)
    reconstructed = layer_norm_reverseCom(y, ln_multi, front_fea=expected)
    torch.testing.assert_close(
        reconstructed[..., [0, 2]],
        torch.zeros_like(reconstructed[..., [0, 2]]),
        atol=atol,
        rtol=0,
    )
    torch.testing.assert_close(ln_multi(reconstructed), y, atol=atol, rtol=rtol)
    print("[PASS] LayerNorm multiple zero weights -> zero reconstructed positions")

    # Near-zero affine weight uses the same stable branch.
    ln_small = nn.LayerNorm(4)
    with torch.no_grad():
        ln_small.weight.copy_(torch.tensor([1.0, 1e-14, -2.0, 0.5]))

    expected = torch.randn(2, 3, 4)
    y = ln_small(expected)
    reconstructed = layer_norm_reverseCom(
        y, ln_small, front_fea=expected, min_abs_weight=1e-12
    )
    assert torch.isfinite(reconstructed).all()
    torch.testing.assert_close(
        reconstructed[..., 1], torch.zeros_like(reconstructed[..., 1]), atol=atol, rtol=0
    )
    print("[PASS] LayerNorm near-zero weight -> zero reconstructed position")

    # External impossible target at a zero-weight affine position is ignored there;
    # other attainable output coordinates remain reproduced.
    y_bad = y.clone()
    y_bad[..., 1] += 10.0
    reconstructed = layer_norm_reverseCom(
        y_bad, ln_small, front_fea=expected, min_abs_weight=1e-12
    )
    out = ln_small(reconstructed)
    valid = torch.tensor([True, False, True, True])
    torch.testing.assert_close(out[..., valid], y_bad[..., valid], atol=atol, rtol=rtol)
    torch.testing.assert_close(
        out[..., 1], ln_small.bias[1].expand_as(out[..., 1]), atol=atol, rtol=rtol
    )
    print("[PASS] LayerNorm impossible zero-weight target -> affine bias")

    # Multi-dimensional normalized_shape.
    ln_2d = nn.LayerNorm((2, 3))
    with torch.no_grad():
        ln_2d.weight[0, 1] = 0.0

    expected = torch.randn(2, 4, 2, 3)
    y = ln_2d(expected)
    reconstructed = layer_norm_reverseCom(y, ln_2d, front_fea=expected)
    torch.testing.assert_close(
        reconstructed[..., 0, 1],
        torch.zeros_like(reconstructed[..., 0, 1]),
        atol=atol,
        rtol=0,
    )
    torch.testing.assert_close(ln_2d(reconstructed), y, atol=atol, rtol=rtol)
    print("[PASS] LayerNorm multidimensional normalized_shape")

    print("[PASS] all normalization reverse tests")
