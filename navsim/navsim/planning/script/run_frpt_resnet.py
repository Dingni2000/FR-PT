...
def _needs_backbone_stage_outputs(recons_keys):
    return any(str(key).startswith("recons_resnet.") for key in recons_keys)


def _enable_backbone_stage_outputs_if_needed(cfg, recons_keys):
    if _needs_backbone_stage_outputs(recons_keys):
        cfg.agent.config.backbone_feature_capture = "stages"
        cfg.agent.config.return_intermediate_features = True


def _call_get_recons_fea(model, features, trajectory, recons_keys, residual_iter):
    """Call current/future get_recons_fea variants without forcing one signature."""
    signature = inspect.signature(model.get_recons_fea)
    kwargs = {}
    if "recons_keys" in signature.parameters:
        kwargs["recons_keys"] = recons_keys
    if "recons_key" in signature.parameters and len(recons_keys) == 1:
        kwargs["recons_key"] = recons_keys[0]
    ...
    if isinstance(recons, dict):
        return recons
    if len(recons_keys) != 1:
        raise TypeError("get_recons_fea returned a tensor, but recons_keys does not identify exactly one key")
    return {recons_keys[0]: recons}
...
def _run_post_train_worker(worker_id, cfg_container, tasks, device_index):
    ...
    del agent_for_keys
    _enable_backbone_stage_outputs_if_needed(cfg, recons_keys)

    logger.info(
        "FRPT ResNet offline worker %d starts %d tasks on %s: %s",
        worker_id,
        len(tasks),
        device,
        tasks,
...
def save_resnet_recons_h5(
    ...
    try:
        with h5py.File(h5_path, "w") as h5_file:
            h5_file.attrs["format"] = "frpt_resnet_v1"
    ...
    logger.info("Saved ResNet FRPT h5 to %s", h5_path)
...
def main(cfg: DictConfig) -> None:
    ...
    if mode in ("post_train", "all"):
        ...
        logger.info("Total FRPT ResNet offline post-train tasks: %d", len(tasks))
        if task_workers > 1:
            ...
            if task_workers > 1:
                ...
                if failed_workers:
                    raise RuntimeError(f"FRPT ResNet offline worker failed, pids={failed_workers}")
                return
        ...
        logger.info("FRPT ResNet finished. Generated checkpoint groups:")
        for row in summary:
            logger.info(
